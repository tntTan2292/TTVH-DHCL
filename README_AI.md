...
`NETWORK-MANAGEMENT-002` (Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2) is `DISCOVERY + PLANNING COMPLETE` — no product code changed yet; awaiting explicit Product Owner/CTO go-ahead before implementation begins.

Current project state is owned by `PROJECT_SNAPSHOT.md`.

Current active handoff: active ticket is `NETWORK-MANAGEMENT-002` (Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2), activated `2026-08-10` immediately after `NETWORK-MANAGEMENT-001` closed `COMPLETED / PO FINAL PASS / CLOSED` on the same date (Section 36 of that manifest, Section 26 of its checkpoint) — `NETWORK-MANAGEMENT-001` is not reopened by this new ticket. Product Owner locked scope directly in the activation prompt: exactly one integrated read-only map screen; data read directly from the two existing source modules (Mạng điểm phục vụ, Mạng đường thư cấp 2), no data duplication; independently toggleable layers for each; the two source screens' already-approved marker/popup/legend/routing/interaction behavior preserved verbatim; no new database table; no dedicated Import/Export/History/Rollback for this screen; no behavior change to the two original screens; the F1.3 Shipment Detail/Evidence screens not folded in. Claude Code performed read-only, delta-only discovery only in this round: both source screens already read via existing `admin`+`viewer`-readable API endpoints (zero backend/schema/migration work needed); both source Map components are self-contained with no existing reusable layer component; shared pure modules (`mapStyles.js`, `roadRoutingService.js`, `routeJourneyGeometry.js`) are directly reusable; the ĐTC2 layer's full approved routing/journey-visual interaction is the main implementation-complexity item; nav/routing wiring follows an established pattern. Two implementation options were identified (manifest Section 8) — Claude Code recommends and will default to Option B (zero-touch to the two original source files) absent further Product Owner/CTO direction. No product code, schema, or database change was made in this discovery round; `Data QLML/` and both stashes confirmed untouched. See `docs/10_TICKETS/NETWORK-MANAGEMENT-002_MANIFEST.md` and its checkpoint for the full record.

...
| POF-TODAY-002-01 | 2026-07-16 | Daily trend path still used the legacy KPI column instead of KPI 2026 | DEFECT | Leadership Dashboard | Daily Trendline | `/f13/dashboard` | `TODAY-002 Daily Trend Data Adapter` | Recovery reopened for source-column correction | DEFECT | Daily trend aggregation must use the approved KPI 2026 source column while preserving one-row-per-date, missing-day handling, returned-shipment NULL semantics, ascending order, and optional BCVH filter | The runtime adapter aggregated from the legacy KPI column, so days where legacy and KPI 2026 differed could show the wrong quality rate | Distorts leadership trendline decisions | High | Yes | Correct the daily trend SQL to use the KPI 2026 field and recheck runtime evidence | `TODAY-002-R1 KPI 2026 Source Column Recovery` | Current recovery ticket | Recheck `getDailyTrendData` against real dates and mixed-column fixture | `docs/06_REVIEWS/Import/TODAY-002_DAILY_TREND_DATA_ADAPTER_REVIEW.md` | Live DB reconciliation and regression test will show differing legacy vs KPI 2026 results | PO WARNING | OPEN | |

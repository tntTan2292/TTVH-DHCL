...
| POF-TODAY-004-01 | 2026-07-17 | Product Owner rejected the separate Quality Delivery Rate Trendline and Volume Trendline presentation | DEFECT | Leadership Dashboard | Quality and Volume Trendline | `/f13/dashboard` | `TODAY-004 Volume Trendline` | Technical PASS / Runtime PASS / PO FAIL | DEFECT | Operation Dashboard should show one professional 30-day combo chart combining total shipment volume and quality rate | The dashboard showed two separate trendline chart sections, creating a rejected PO review experience | Blocks TODAY-004 product acceptance and downstream TODAY-005 activation | High | Yes | PO recheck the implemented combo chart and keep TODAY-005 inactive | `TODAY-004-R1 Quality and Volume Combo Trendline Recovery` | Ready for PO recheck before TODAY-005 | Recheck Vietnamese chart title, one daily-trend request, dual axes, dashed 90% target, two-decimal tooltip fields, BCVH filter, and missing-date gaps | `docs/06_REVIEWS/Import/TODAY-004-R1_QUALITY_AND_VOLUME_COMBO_TRENDLINE_RECOVERY.md` | Tests PASS; build PASS; lint PASS with existing warnings only; browser rendered one Vietnamese combo chart with 30 teal bars, one dark-blue quality line, and dashed red target | API `daily-trend` returned 30 rows for `2026-06-16` through `2026-07-15`; authenticated browser rendered `/f13/dashboard` combo chart titled `Sản lượng và chất lượng phát – 30 ngày` | PO CHECK REQUIRED | READY FOR PO RECHECK | |
| POF-TODAY-004-02 | 2026-07-17 | Product Owner reported BCVH filter and combo trendline runtime blockers during TODAY-004-R1 recheck | DEFECT | Leadership Dashboard | Operation Dashboard filters and combo trendline | /f13/dashboard | TODAY-004-R1 Quality and Volume Combo Trendline Recovery | Technical PASS / Runtime PASS / PO FAIL | DEFECT | Operation Dashboard must list exactly six SSOT BCVH units plus Tất cả BCVH, remove the meaningless status selector, and keep the 30-day combo chart visible for each valid BCVH | Dashboard had placeholder BCVH options, retained the status selector, and selected BCVH could make the combo chart disappear | Blocks TODAY-004 product acceptance and downstream TODAY-005 activation | High | Yes | Activate TODAY-004-R2 and keep TODAY-005 inactive | TODAY-004-R2 BCVH Filter and Combo Trendline Recovery | Ready for PO recheck before TODAY-005 | Recheck six BCVH options, absent status selector, each canonical ma_bcvh, stable chart card, and aggregate reset | docs/06_REVIEWS/Import/TODAY-004-R2_BCVH_FILTER_AND_COMBO_TRENDLINE_RECOVERY.md | Tests PASS; build PASS; lint PASS with existing warnings only; API validation PASS for six canonical BCVH codes | Authenticated browser validation required for final PO review; technical runtime evidence recorded in R2 review | PO CHECK REQUIRED | READY FOR PO RECHECK | |
| POF-TODAY-004-03 | 2026-07-17 | Product Owner screenshot showed the BCVH dropdown contained only `Tất cả BCVH` | DEFECT | Leadership Dashboard | Operation Dashboard BCVH filter | `/f13/dashboard` | `TODAY-004-R2 BCVH Filter and Combo Trendline Recovery` | Technical PASS / Runtime PASS / PO FAIL | DEFECT | The BCVH dropdown must contain exactly `Tất cả BCVH` plus six canonical BCVH units from stable metadata | PO browser showed only the aggregate option, contradicting R2 browser PASS evidence | Blocks TODAY-004 product acceptance and downstream TODAY-005 activation | High | Yes | Activate TODAY-004-R3 and keep TODAY-005 inactive | `TODAY-004-R3 Canonical BCVH Options Runtime Recovery` | Ready for PO recheck before TODAY-005 | Recheck metadata payload, seven DOM options, dropdown screenshot, six selections, and aggregate reset | `docs/06_REVIEWS/Import/TODAY-004-R3_CANONICAL_BCVH_OPTIONS_RUNTIME_RECOVERY.md` | Backend tests PASS; frontend tests PASS; build PASS; lint PASS with existing warnings only; `/dashboard/meta` no-store payload returns six canonical units | Authenticated browser DOM recorded seven options and chart visible for all six selections; screenshots attached in `docs/06_REVIEWS/Import/screenshots/` | PO CHECK REQUIRED | READY FOR PO RECHECK | |

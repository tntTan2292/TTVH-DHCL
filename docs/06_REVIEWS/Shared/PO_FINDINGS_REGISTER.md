...
| POF-DA-IMPL-003-01 | 2026-07-18 | D-1 and D-7 comparison data is correct but not sufficiently visible for leadership use without tooltip hover | DEFECT | Leadership Dashboard | Integrated Trend and Risk Workspace | `/f13/dashboard` | `DA-IMPL-003 Integrated Trend and Risk Workspace` | `PO WARNING` remediation implemented, Product Owner recheck pending | DEFECT | Leadership comparison must visibly show current value, comparison value, absolute delta, direction, and semantic meaning for total volume, pass rate, and failed shipment count; 7-day mode must expose per-day D-7 evidence without relying on tooltip hover | Previous implementation required too much hover/detail reading to understand D-1 and D-7 differences immediately | Blocks DA-IMPL-003 product acceptance until PO recheck | High | Yes | Product Owner rechecks the revised `So sánh điều hành` widget and `Bằng chứng so cùng kỳ 7 ngày` table | `DA-IMPL-003 Integrated Trend and Risk Workspace` | Active ticket remediation before any next-ticket activation | Recheck aggregate `2026-07-15` and BCVH `533140` on `/f13/dashboard` | `docs/06_REVIEWS/Dashboard/DA-IMPL-003_INTEGRATED_TREND_AND_RISK_WORKSPACE.md` | Runtime preview `http://127.0.0.1:5181/f13/dashboard`; aggregate D-1 `3.677/+543`, `67.20%/+6.80 điểm %`, `1.037/-31`; aggregate D-7 `3.677 vs 3.688`, `67.20% vs 66.57%`, `1.037 vs 1.092`; BCVH `533140` D-1 and D-7 values visible; console errors none | PO WARNING | Ready for Recheck | |
| POF-DA-IMPL-003-02 | 2026-07-18 | Value shown as `chưa phân loại` is actually `Bưu gửi chuyển hoàn`, and the Integrated Trend area still spent visual weight on a redundant `Tỷ lệ không đạt` quality-rate story | DEFECT | Leadership Dashboard | Unified Command Summary and Integrated Trend and Risk Workspace | `/f13/dashboard` | `DA-IMPL-003 Integrated Trend and Risk Workspace` | `PO WARNING` remediation implemented, Product Owner recheck pending | DEFECT | Dashboard must present `Chuyển hoàn`; preserve `Đạt + Không đạt + Chuyển hoàn = Tổng mẫu đo kiểm`; use only `Tỷ lệ đạt` as the primary quality rate; keep `Không đạt` and `Chuyển hoàn` as compact supporting composition/evidence | Previous UI treated residual population as unknown/unclassified and showed both pass-rate and failed-rate trend stories | Blocks DA-IMPL-003 product acceptance until PO recheck | High | Yes | Product Owner rechecks Command Summary wording, measurement composition, and Integrated Trend chart simplification | `DA-IMPL-003 Integrated Trend and Risk Workspace` | Active ticket remediation before any next-ticket activation | Recheck aggregate `2026-07-15` and BCVH `533140` on `/f13/dashboard` | `docs/06_REVIEWS/Dashboard/DA-IMPL-003_INTEGRATED_TREND_AND_RISK_WORKSPACE.md` | Data-contract check PASS: aggregate `2.471 + 1.037 + 169 = 3.677`; BCVH `533140` `1.252 + 373 + 69 = 1.694`; targeted tests prove mismatch exposure and removed failed-rate chart source | PO WARNING | Ready for Recheck | |
| POF-DA-IMPL-003-03 | 2026-07-18 | `So sánh điều hành` still required switching modes, while leadership must see D-1 and D-7 simultaneously | DEFECT | Leadership Dashboard | Integrated Trend and Risk Workspace | `/f13/dashboard` | `DA-IMPL-003 Integrated Trend and Risk Workspace` | `PO WARNING` remediation implemented, Product Owner recheck pending | DEFECT | Two compact comparison widgets must show simultaneously: `So với hôm qua` and `So với cùng kỳ tuần trước`; each widget exposes only `Tỷ lệ đạt` and `Sản lượng` with current value, comparison value, delta, and increase/decrease direction | Previous comparison area used mode tabs/toggles and still carried extra failed-count comparison detail | Blocks DA-IMPL-003 product acceptance until PO recheck | High | Yes | Product Owner rechecks simultaneous D-1/D-7 comparison widgets above the trend chart | `DA-IMPL-003 Integrated Trend and Risk Workspace` | Active ticket remediation before any next-ticket activation | Recheck aggregate and BCVH contexts on `/f13/dashboard` | `docs/06_REVIEWS/Dashboard/DA-IMPL-003_INTEGRATED_TREND_AND_RISK_WORKSPACE.md` | Targeted tests PASS: simultaneous widgets, D-1/D-7 calculations, missing D-1/D-7 states, aggregate/BCVH request context, and pass-rate/volume-only widget data | PO WARNING | Ready for Recheck | |

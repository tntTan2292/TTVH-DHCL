# TODAY-006-R2 PO Acceptance Checklist

## 1. Purpose

Verify aggregate KPI normalization, invalid-code rejection, and real authenticated browser evidence for the restored dashboard surfaces.
...
- Required dataset or seed state: KPI data with aggregate and scoped BCVH values, timeline data with missing and empty states, and accepted daily-trend payload
...
- Expected Result: Exactly one shared `daily-trend` request is issued.
...
- After FAIL: create or update the responsible recovery work and keep `TODAY-006-R2` open.

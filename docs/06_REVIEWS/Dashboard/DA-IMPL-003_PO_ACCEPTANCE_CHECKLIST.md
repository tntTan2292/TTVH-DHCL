...
| `Chuyển hoàn` replaces the previous `chưa phân loại` presentation | `READY FOR PO RECHECK` | Mapper tests and source checks show `total_unknown` is presented as `Chuyển hoàn` for compatibility |
| Measurement equation is preserved | `READY FOR PO RECHECK` | Aggregate `2.471 + 1.037 + 169 = 3.677`; BCVH `533140` `1.252 + 373 + 69 = 1.694` |
...
| Chart includes volume bars, one `Tỷ lệ đạt` line, and target/reference line | `READY` | Source and targeted tests confirm `Tỷ lệ không đạt` line/legend are removed |

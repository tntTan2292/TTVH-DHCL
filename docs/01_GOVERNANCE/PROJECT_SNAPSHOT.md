...
| PO Product Status | `F13-BCVH-RANKING-REDESIGN-PLAN COMPLETED / HANDOFF; F13-BCVH-RANKING-REDESIGN-IMPL PO APPROVED SCOPE - IMPLEMENTATION PENDING; F13-INTERNAL-ROUTE-AUDIT COMPLETED / PO PASS; F13-DATA-QUALITY-001 DEFERRED / PRESERVED; F13-SHIPMENT-001 DEFERRED / PRESERVED; DA-IMPL-008 COMPLETED / PO PASS; AUTO-IMPORT-009 COMPLETED / PO PASS` |
...
Current handoff: `F13-BCVH-RANKING-REDESIGN-PLAN` is `COMPLETED / HANDOFF` and the Product Owner approved the BCVH Ranking redesign scope. The active ticket is now `F13-BCVH-RANKING-REDESIGN-IMPL READY FOR IMPLEMENTATION`. Preserve current day vs previous day vs comparison day, delayed cash handover count, participating postman-route count, route quality distribution by Dashboard color bands, Dashboard SSOT, semantic colors, existing business thresholds, and the `7` PO-confirmed non-postman/customer-pickup route exclusions from postman counts. `F13-INTERNAL-ROUTE-AUDIT` remains `COMPLETED / PO PASS`: Route Ranking uses exact filters `Tuyen buu ta | Tat ca`; default is `Tuyen buu ta`; the active page includes a minimal runtime-backed `Bang Tuyen Ranking`; confirmed non-postman/customer-pickup rows show `Nhan tai buu cuc`; Hue scope excludes route codes not starting with `53`. Product Owner deferred `F13-DATA-QUALITY-001` and `F13-SHIPMENT-001` implementation while preserving both manifests. Do not expand implementation beyond the new BCVH Ranking redesign manifest.

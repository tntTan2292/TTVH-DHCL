...
| `docs/10_TICKETS/AUTO-BACKFILL-RUNTIME_MANIFEST.md` | Ticket Manifest | AUTO-BACKFILL-RUNTIME (Ticket 7, open): activation/readiness record, four verified automated lanes, unchanged Safety contract, Section 15 backlog-cleanup ("Option A"), and Section 49 the F41-AUTOBF-SQLITE-BUSY-01 incident (SQLITE_BUSY defect found/fixed, PO-confirmed real single-date/single-lane run) `CLOSED / PO-CONFIRMED` 2026-09-07. | L2 | Current Required Reading | When AUTO-BACKFILL-RUNTIME is the active work item. | Mandatory |
| `docs/06_REVIEWS/Import/AUTO-BACKFILL-RUNTIME_CHECKPOINT_001.md` | Checkpoint | AUTO-BACKFILL-RUNTIME checkpoint: activation record, Claude-to-Codex continuation ledger, independently verified four-lane registry, read-only operational audit, safety evidence, PO's one-day/one-source runtime sequence, and Section 8 the F41-AUTOBF-SQLITE-BUSY-01 incident closure. | L2 | Current Required Reading | When AUTO-BACKFILL-RUNTIME is the active work item. | Mandatory |
| `docs/04_TECHNICAL_PLANNING/Feature/AB-CALENDAR-01_HOLIDAY_CALENDAR_DESIGN.md` | Feature Design | PO-approved design of record for AB-CALENDAR-01 (LỊCH NGHỈ, 2026-08-27, commit `15628be1`): a shared, indicator-agnostic holiday calendar overlaid on the coverage scan with precedence SUCCESS > ACTIVE exception > LỊCH NGHỈ, the 8-point risk review (frozen 6-state model, blast radius, `VERIFIED_NO_DATA` collision, queue divergence), and the four Product Owner decisions. Implemented in manifest Section 39. AB-CALENDAR-01 ticket closed / PO UI CHECK PASS 2026-08-28 (manifest Section 48). | L4 | Archive | Conditional reference for AB-CALENDAR-01 history only. | Reference |
| `docs/04_TECHNICAL_PLANNING/Feature/AB-CALENDAR-01_4_STATUS_MODEL_DESIGN.md` | Feature Design | Design of record for AB-CALENDAR-01 (approved 2026-08-27, implemented 2026-08-28, PO UI CHECK PASS / CLOSED 2026-08-28): the 4 PO-facing coverage statuses (COMPLETED / INCOMPLETE / EXCLUDED / DATA_ERROR) replacing the frozen 6-state model, plus PO's operational-completion delta (processed = COMPLETED+EXCLUDED, unprocessed = INCOMPLETE+DATA_ERROR). Carries the full mapping, queue and selection rules, measured live impact, file scope, test plan, acceptance criteria, and all four approved decisions D1-D4 (narrow include_excluded opt-in; LEGACY_DATA_PRESENT_WITHOUT_EVIDENCE maps to COMPLETED; frozen UI plan amended 6 to 4; DATA_ERROR stays selectable) — all four implemented and PO-accepted. See `docs/10_TICKETS/AUTO-BACKFILL-RUNTIME_MANIFEST.md` Section 48 for the closure evidence. | L4 | Archive | Conditional reference for AB-CALENDAR-01 history only. | Reference |
| `docs/10_TICKETS/AUTO-BACKFILL-UI-REMEDIATION_MANIFEST.md` | Ticket Manifest | Closed ticket AUTO-BACKFILL-UI-REMEDIATION (Phase B frontend, `CLOSED / PO UI CHECK PASS`, 2026-08-20, baseline `29346c92`, executor Antigravity; 2 backend deltas `PO BACKEND GATE PASS`): No-code Operator UI for 6 canonical coverage statuses, smart monthly grouping accordions, real PO exception confirmation/revocation modals (`PO_EXEMPTED`), slide-out audit drawers, and VNPost Light Dashboard tokens. | L2 | Historical / Last Closed Ticket | When the Phase B closure record is needed. | High |
| `docs/06_REVIEWS/Import/AUTO-BACKFILL-UI-REMEDIATION_CHECKPOINT_001.md` | Checkpoint | Closed checkpoint for AUTO-BACKFILL-UI-REMEDIATION: activation record, implementation ledger, frontend verification suite, the 2 backend deltas, and the Section 6 PO closure record. | L2 | Historical / Last Closed Checkpoint | When the Phase B closure evidence is needed. | High |
| `docs/10_TICKETS/AUTO-BACKFILL-UI_MANIFEST.md` | Ticket Manifest | Parent ticket AUTO-BACKFILL-UI (`READY FOR PO FINAL PLAN APPROVAL`, 2026-08-19, baseline `ef7cbe85`): No-code Operator UI for Auto Backfill V2 platform. Holds ticket authority, 3-tab architecture, 10 rows/page default pagination, right drawer event log model, neutral naming, and 4+ indicator extensibility contract. | L2 | Current Required Reading | Current ticket only. | Mandatory |
| `docs/10_TICKETS/AUTO-BACKFILL-UI_PLAN.md` | Ticket Plan | Official UI/UX Audit & Redesign Plan for AUTO-BACKFILL-UI (`READY FOR PO FINAL PLAN APPROVAL`, 2026-08-19): full audit findings, 3-tab IA, zero-infinite-scroll 10-rows/page pagination, locked Slide-out Right Drawer for queue audit events, Tab 3 for legacy manual import history, 4+ indicator future-state proof (`F1.3`, `F4.1`, `F2.TEST`, `F5.TEST`), and minimal read-only additive backend Coverage API metadata scope (`backend/src/controllers/autoBackfillCoverageController.js` and `backend/src/services/autoBackfillCoverageService.js`). **Section 4 amended 2026-08-27** under approved frozen-document delta D3: the 6 PO-facing coverage states are replaced by 4 (`COMPLETED` / `INCOMPLETE` / `EXCLUDED` / `DATA_ERROR`), with the original 6-state table preserved verbatim under a `SUPERSEDED` heading in Section 4.2. | L2 | Current Required Reading | Current ticket only. | Mandatory |
| `docs/06_REVIEWS/Import/AUTO-BACKFILL-UI_CHECKPOINT_001.md` | Checkpoint | Current checkpoint for AUTO-BACKFILL-UI: Gate 5 PASS confirmation, remediation logs, UI/UX audit findings, 3-tab architecture, 4-indicator scalability proof, 3-point remediation summary, and final plan approval handoff. | L2 | Current Required Reading | Current ticket only. | Mandatory |
| `docs/10_TICKETS/AUTO-BACKFILL-COVERAGE-EXCEPTION_MANIFEST.md` | Ticket Manifest | Closed ticket AUTO-BACKFILL-COVERAGE-EXCEPTION (Phase A backend, `CLOSED / PO BACKEND GATE PASS`, 2026-08-19, baseline `3fce810f`, executor Claude Code): PO-approved 6-state coverage model, controlled Legacy Baseline Reconciliation, 5-point-proof `VERIFIED_NO_DATA`, reasoned/audited/reversible `PO_EXEMPTED`. **Historical:** those 6 states were superseded on 2026-08-27 by the 4 PO-facing statuses (`AUTO-BACKFILL-UI_PLAN.md` Section 4); they survive only as internal technical classifications. | L2 | Historical / Closed Ticket | When the Phase A backend contract is needed. | High |
| `docs/10_TICKETS/COORDINATOR-ROLE-MIGRATION_MANIFEST.md` | Ticket Manifest | Documentation-only ticket COORDINATOR-ROLE-MIGRATION (`IMPLEMENTED / READY FOR PO CHECK`, 2026-08-19, baseline `29346c92`, executor Claude Code): renames the CTO/Coordinator/Technical Decision Authority role from `ChatGPT` to `Claude` (claude.ai chat) across `docs/01_GOVERNANCE/`, `README_AI.md`, `CLAUDE.md`, preserving frozen decision-log rows, archived documents, and every other role/rule unchanged. | L2 | Current Required Reading | Current ticket only. | Mandatory |
| `docs/06_REVIEWS/Governance/COORDINATOR-ROLE-MIGRATION_CHECKPOINT_001.md` | Checkpoint | Checkpoint for COORDINATOR-ROLE-MIGRATION: activation, self-verified discovery, the 2 exception classes preserved and why, implementation ledger, validation evidence, PO-check handoff. | L2 | Current Required Reading | Current ticket only. | Mandatory |
| `docs/06_REVIEWS/Import/AUTO-BACKFILL-COVERAGE-EXCEPTION_CHECKPOINT_001.md` | Checkpoint | Current checkpoint for AUTO-BACKFILL-COVERAGE-EXCEPTION: activation record, locked scope, and implementation/validation evidence ledger. | L2 | Current Required Reading | Current ticket only. | Mandatory |
...
## F13-BCVH-RANKING-OVERVIEW-01 Design Registration - 2026-08-28

Append-only update: the design of record for the Product Owner-approved BCVH Ranking overview extension is registered, and `F13-STANDARDIZATION-001_MANIFEST.md` becomes the active manifest again by way of its appended Section 35. This section supersedes earlier F1.3 current-ticket rows for live onboarding; historical entries remain preserved, and `PROJECT_SNAPSHOT.md` owns mutable live state.

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-BCVH-RANKING-OVERVIEW-01_DESIGN.md` | Design of Record | Approved design for the BCVH Ranking overview extension: objective and the 9 locked PO decisions; read-only verification evidence against real code and the real database; the five blocks (monthly trend, daily detail, MTD summary, route capacity, the unchanged daily ranking table); time rules (anchor date, per-block periods, start-of-month, three missing-data states); the `/f13/ranking/bcvh/overview` contract with its four fixed aggregate queries and four binding constraints; measured performance and `RISK-PERF-01`; file scope; the B1/F1/I1 phase split; test plan T1-T11 and F1-F8 with the mandatory regression pair; acceptance criteria AC-01..AC-17; and real-data risks `RISK-DATA-01..03`, `RISK-PERF-01`, `RISK-SCOPE-01`. | L2/L3 | Current Required Reading | Mandatory before executing any phase of `F13-BCVH-RANKING-OVERVIEW-01`. | Mandatory |
...
## F13-ROUTE-RANKING-PERIOD-01 Design Registration - 2026-08-28

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-ROUTE-RANKING-PERIOD-01_DESIGN.md` | Design of Record | Design of record for `F13-ROUTE-RANKING-PERIOD-01` (Tuyến Ranking theo kỳ và đối soát phạm vi), Revision R0, `AWAITING PO APPROVAL`: objective and the 7 locked PO decisions; the two audit defects the design must close (`DEF-01` date-range filter silently collapsing to the last day, `DEF-02` route figures not reconciling with BCVH); mandatory vocabulary rules (the term "MTD" is banned from UI, code, comments and tests); the measurement-instance denominator and the `rate = null ⟺ volume = 0` rule inherited from the BCVH ticket's R1-B; anchor-date time rules including `T-01` (route list is the union over the month, not the anchor day — 35 vs 30 routes measured); the four-group scope reconciliation with its checkable identity, verified true on real data for both periods; the new `/f13/ranking/route/periods` contract with 5 binding constraints and 4 fixed queries; measured performance; UI plan reusing the existing table and route-detail panel rather than copying BCVH Ranking's blocks; file scope and the B1/F1/I1 phase split; test plan T1-T13, R1-R4, F1-F16; acceptance criteria AC-01..AC-14 and PO-01..PO-08; risks `RISK-DATA-01..06`, `RISK-PERF-01`, `RISK-SCOPE-01`; and the one open decision `D-OPEN-01`. | L2/L3 | Current Required Reading | Mandatory before executing any phase of `F13-ROUTE-RANKING-PERIOD-01`. | Mandatory |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 46 (appended 2026-08-28) activates `F13-ROUTE-RANKING-PERIOD-01` and records its ticket header, origin (the completed read-only audit and the Product Owner's selection of Phương án B), the 7 PO decisions, the two defects `DEF-01`/`DEF-02`, the contract summary and the B1/F1/I1 phasing. Sections 1-45 unchanged; Section 45's closure of `F13-BCVH-RANKING-OVERVIEW-01` stands. | L2 | Active Onboarding | Current ticket only. | Mandatory |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-BCVH-RANKING-OVERVIEW-01_DESIGN.md` | Design of Record | Unchanged and still `IMPLEMENTED / PO PASS / CLOSED`. Demoted from Current Required Reading by this registration — read only for BCVH Ranking Overview history, or when reusing its `R1-B` null-rate rule, which `F13-ROUTE-RANKING-PERIOD-01` inherits deliberately. | L4 | Archive | Conditional reference only. | Reference |

Current state: `F13-ROUTE-RANKING-PERIOD-01 DESIGN OF RECORD WRITTEN / AWAITING PO APPROVAL`. This registration grants no implementation authority — Phase B1 requires explicit Product Owner approval of the design of record. No code, database, schema or API changed. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `F13-ROUTE-EVIDENCE-STATUS-02` (the Đạt/Lỗi Evidence drill-down split out by PO decision 7) is recorded on the roadmap but not opened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 PO Approval / D-OPEN-01 Resolution - 2026-08-28

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-ROUTE-RANKING-PERIOD-01_DESIGN.md` | Design of Record | Now at `Revision: R1 (2026-08-28)`, `PO APPROVED / READY FOR IMPLEMENTATION`. Product Owner approved the design and additionally resolved `D-OPEN-01`: "Cùng kỳ tháng trước" reuses BCVH Ranking's own same-elapsed-days formula verbatim (`_getBcvhOverviewAggregate('mtd', ...)`, capped at the previous month's last day — anchor `20/8` compares `01-20/8` against `01-20/7`), UI label `Cùng kỳ tháng trước`, `MTD` stays banned everywhere. Every reference to the previous-period comparison — vocabulary (§3.1), the difference formula (§3.3), the period definition and its reused formula (§4.2/§4.2.1, new), the anchor-day-01 edge case (§4.3), the payload example with real re-measured route figures (§6.3), the query description (§6.5), the UI labels (§7.3/§7.5), the performance table (§8, same-window query measured 93 ms vs. 272 ms for the full-month version) and test `T12` (§10.1) — was synchronized in this same revision. §14 is now a closure log recording the R0→R1 decision trail; the R0 comparison table (trọn tháng 66.47% vs. cùng số ngày 66.73%, 0.26pp apart) is preserved as historical record, not deleted. New Revision R1 § numbering: 15 sections total (§14 renamed, §15 status). | L2/L3 | Current Required Reading | Mandatory before executing any phase of `F13-ROUTE-RANKING-PERIOD-01`; supersedes the Revision R0 registration above for onboarding purposes — read this entry's description, not the R0 one, for the current state. | Mandatory |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 47 (appended 2026-08-28) records the Product Owner's approval of the design of record and the `D-OPEN-01` resolution, including the technical verification performed before approval (real-database confirmation of the reused MTD-capping formula's edge-case behavior, and the re-measured 93 ms query cost). Sections 1-46 unchanged; Section 46's activation record stands. | L2 | Active Onboarding | Current ticket only. | Mandatory |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = PO APPROVED / READY FOR IMPLEMENTATION` (Revision R1). Phase B1 (backend, additive-only, Claude Code / Sonnet) is authorized to start per manifest Section 47. No code, database, schema or API changed by this approval. `D-OPEN-01` = `RESOLVED`; no open Product Owner decision remains in this ticket. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `F13-ROUTE-EVIDENCE-STATUS-02` is recorded on the roadmap but not opened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 Phase B1 Implementation - 2026-08-28

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `backend/src/repositories/FactBuuGuiRepository.js` | Repository | Three additive methods for `F13-ROUTE-RANKING-PERIOD-01`: `getRoutePeriodDailyFacts()` (Q2), `getRoutePeriodPreviousMonth()` (Q3, reuses BCVH Ranking's own MTD same-elapsed-days capping formula verbatim), `getRouteScopeReconciliation()` (Q4, the AC-05 four-group identity). `getRouteRanking`/`getRouteRankingFacts`/`getBcvhRanking`/`getEvidenceList*` unchanged. | L3 | Active Code | When touching Route Ranking period/reconciliation queries. | High |
| `backend/src/services/routePeriodService.js` | Service | New — orchestrates the 4 fixed queries (Q1 inline via `config/db`'s `all()`, Q2-Q4 via the repository), the Node-side month roll-up (C-02), null-iff-zero-volume rate rule (C-04), and the tied-last ranking rule (AC-08). Constructor-injectable `repository`/`queryAnchor` for dependency-free unit testing. | L3 | Active Code | When touching Route Ranking period logic. | High |
| `backend/src/controllers/DashboardController.js` | Controller | New `getRoutePeriods` handler for `GET /f13/ranking/route/periods`; every existing handler unchanged. | L3 | Active Code | When touching the F13 dashboard controller. | Medium |
| `backend/src/routes/f13Routes.js` | Route | New `GET /ranking/route/periods` line, same role gate as `/ranking/route`; existing line unmodified. | L3 | Active Code | When touching F1.3 route registration. | Medium |
| `backend/src/repositories/FactBuuGuiRepository.routePeriod.test.js` | Test | 5 tests, real SQL against in-memory `node:sqlite`, covers Q2/Q3/Q4 including the shorter-previous-month capping edge case and the AC-05 identity on synthetic data. | L3 | Active Test | When touching the repository additions. | High |
| `backend/src/services/routePeriodService.test.js` | Test | 12 tests, fake repository/queryAnchor, covers T1-T13 roll-up/ranking/null rules and the T13 fixed-4-DB-touches guarantee. | L3 | Active Test | When touching the service. | High |
| `backend/src/controllers/DashboardController.routePeriods.test.js` | Test | 4 tests, controller wiring and route registration. | L3 | Active Test | When touching the controller/route. | Medium |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 48 (appended 2026-08-28) records Phase B1's implementation, full test evidence (21/21 new, 36/36 regression, 454/468 full sweep with zero attributable regression), and the real-data read-only validation against all 9 BCVH. Sections 1-47 unchanged. | L2 | Active Onboarding | Current ticket only. | Mandatory |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = PHASE B1 IMPLEMENTED / TECHNICAL PASS`. No Product Owner UI Check required for this phase (design of record §12.2). No database, schema, migration, or frontend file changed. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. Phase F1 (frontend, Antigravity) is authorized but not started. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 Phase I1 Integration Validation - 2026-08-29

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Phase I1 live integration validation: real backend + real frontend + real browser, cross-checked against the live API JSON and the Design of Record. Finds the integration broken — root cause is `RoutePerformancePage.jsx` (landed in the separate Phase F1 commit `8d39a5f9`) fully replacing its `getRouteRanking()` call with `getRoutePeriods()` instead of calling both and merging by `ma_tuyen` per design §7.3. Full evidence with file:line references for each of 12 findings: the scope-reconciliation UI (closing DEF-02) never renders at all; fabricated `0.0%`/`0` on executive KPI cards; the only-failed filter always returns zero rows; every route mislabeled with the wrong classification; a duplicate `XH`/`Hạng` column pair (the exact anti-pattern §7.3 forbade); broken default sort; a direct on-screen data contradiction between the table row and the detail panel for the same route; an `AC-14` literal violation (the string "MTD" in a code comment). Includes a `git worktree` comparison against the pre-Phase-F1 baseline distinguishing genuinely new regressions from pre-existing test staleness. `AC-05` confirmed true at the API layer, not self-approved at the ticket level. | L2/L3 | Current Required Reading | Mandatory before any further work on this ticket, including a Phase F1 remediation round. | Mandatory |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 49 (appended 2026-08-29) records Phase I1's validation and its BLOCKED result. Sections 1-48 unchanged. | L2 | Active Onboarding | Current ticket only. | Mandatory |
| `docs/06_REVIEWS/Shared/F13-ROUTE-RANKING-PERIOD-01-PHASE-F1_CHECKPOINT_001.md` | Checkpoint | Phase F1's own self-written checkpoint (self-stated `COMPLETED / AWAITING PO CHECK`), landed without a manifest section, `DOCUMENT_INDEX.md` entry, or `PROJECT_SNAPSHOT.md` update at the time. Superseded for onboarding purposes by the Phase I1 checkpoint above, which found its claimed completion does not hold up under live integration testing. | L3 | Conditional Reference | Historical record of what Phase F1 claimed to deliver, for comparison against the Phase I1 findings. | Reference |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = PHASE I1 VALIDATION COMPLETE / INTEGRATION DEFECTS FOUND / BLOCKED`. Not ready for PO UI Check. Claude Code does not self-award PO PASS and does not self-approve `AC-05` at the ticket level. Recommended next step (CTO/PO decision): Phase F1 remediation. No product code, database, schema, Evidence, or business rule was changed by this validation round. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 Phase I1 Remediation - 2026-08-29

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Section 12 (appended) records the fix for every defect Section 1-11 found: `RoutePerformancePage.jsx` now calls both `getRouteRanking()` and `getRoutePeriods()` in parallel and merges by `ma_tuyen` via a new `mergeRouteData()` function, per Design §7.3. The scope-reconciliation UI now renders real numbers with a period toggle; the two BCVH-wide KPI cards, the only-failed filter, route classification, and default sort are all fixed; the duplicate `XH` column is removed; the `AC-14` "MTD" violation and all 5 new lint warnings are resolved. Re-validated live against all 9 real BCVH: `AC-05`/`AC-07` now met at the UI layer, not just the API layer. 16 new tests (`routePeriodData.test.js`), zero regressions attributable to this round (verified via `git stash` and exact per-assertion comparison against the pre-existing stale test debt already documented in Section 1-11). | L2/L3 | Current Required Reading | Mandatory before any further work on this ticket, including the Product Owner UI Check. | Mandatory |
| `frontend/src/features/route/routePeriodData.js` | Data Helper | Now also exports `mergeRouteData()` (merges the old day-scoped endpoint with the new periods endpoint by `ma_tuyen`, null-safe for routes absent on the anchor day) and `buildReconciliationView()` (maps the real `reconciliation.day`/`month` contract shape for display). The "MTD" comment removed (`AC-14`). | L3 | Active Code | When touching Route Ranking period/merge logic. | High |
| `frontend/src/features/route/RoutePerformancePage.jsx` | Page | Rewritten to call both endpoints in parallel, render the merged rows, show the scope-reconciliation strip with a period toggle, and remove the duplicate `XH` column. `getRouteRanking()`, `getRoutePeriods()`, all existing filter/sort/pagination logic in `routeRankingCalculations.js`/`routeRankingFilters.js` unchanged. | L3 | Active Code | When touching the Route Ranking screen. | High |
| `frontend/src/features/route/routePeriodData.test.js` | Test | New — 16 tests for `mergeRouteData()`/`buildReconciliationView()` covering positive, genuine-zero, absent-on-anchor-day, missing-old-response, route-union, classification (both filter modes), alias, and `AC-14` cases. | L3 | Active Test | When touching the merge logic. | High |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 50 (appended 2026-08-29) records the remediation and re-validation, `READY FOR PO CHECK`. Section 49 (the original BLOCKED finding) stands unchanged as historical record. | L2 | Active Onboarding | Current ticket only. | Mandatory |
...
## F13-ROUTE-RANKING-PERIOD-01 AC-09 Remediation + Stale-Test Cleanup - 2026-08-29

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Section 13 (appended) closes the two residual items Section 12 disclosed instead of leaving them for a new ticket: `AC-09`'s tooltip (a static caption, not hover-only, explaining `Đạt + Không đạt` can be less than `Sản lượng`) added to the selected-route panel; all 8 pre-existing stale assertions in 4 Route Ranking test files individually re-diffed against the current file and updated to the exact current wording — never deleted or loosened. One genuine, non-stale defect surfaced during the review (a missing `aria-pressed` on the route-type filter buttons) was fixed in code rather than dropped from its test. Full before/after table of all 8 stale-assertion fixes. 2 new tests, 79/79 full targeted Route Ranking sweep, 409/413 full frontend sweep (same 4 pre-existing unrelated failures on record), zero regressions. | L2/L3 | Current Required Reading | Mandatory before any further work on this ticket, including independent technical review and the eventual Product Owner UI Check. | Mandatory |
| `frontend/src/features/route/RoutePerformancePage.jsx` | Page | Added the AC-09 caption under the `Sản lượng phát`/`Đạt chỉ tiêu`/`Không đạt` grid in `RouteSelectedPanel`; restored `aria-pressed={routeType === item.value}` on the route-type filter buttons (a real accessibility regression found while reviewing the stale tests, not a stale-wording item). No formula, field binding, or numeric value changed. | L3 | Active Code | When touching the Route Ranking selected-route panel or the route-type filter. | High |
| `frontend/src/features/route/RoutePerformancePage.volumeReconciliationTooltip.test.js` | Test | New — 2 tests for `AC-09`: the exact caption text and its position directly after the `Sản lượng phát` grid; that the three underlying render call sites are unchanged. | L3 | Active Test | When touching the selected-route panel's day-scoped KPI grid. | High |
| `frontend/src/features/route/RoutePerformancePage.blackReturned.test.js` | Test | 2 stale assertions updated to current wording (`Được ghi nhận phân loại BLACK theo quy chuẩn KPI 2026.`; `dir: 'asc'`; `Chỉ hiện tuyến phát sinh lỗi`); the BLACK/Chuyển hoàn mislabeling guard is now scoped to just that caption's block so it doesn't collide with the new, unrelated AC-09 caption's legitimate use of "chưa có kết quả". | L3 | Active Test | When touching Chuyển hoàn labeling or the default sort. | High |
| `frontend/src/features/route/RoutePerformancePage.delayedCash.test.js` | Test | 4 stale assertions updated to current wording (`label: 'BG Chậm nộp tiền'`; `label: 'Tỷ lệ chậm nộp'`; the current PTC-3h caption text; `dir: 'asc'`; `Chỉ hiện tuyến phát sinh lỗi`). | L3 | Active Test | When touching the delayed-cash column group or panel caption. | High |
| `frontend/src/features/route/RoutePerformancePage.delayedCashWidget.test.js` | Test | 1 test's label/wording assertions updated to current wording (`label: 'BG Chậm nộp tiền'`; `label: 'Tổng tuyến phân tích'`; current PTC-3h caption; `dir: 'asc'`); other 2 tests unchanged. | L3 | Active Test | When touching the executive KPI widgets. | High |
| `frontend/src/features/route/routeRankingFilters.test.js` | Test | 1 test's assertions updated to current wording/structure (`Bảng xếp hạng hiệu năng tuyến`; `useState(ROUTE_BCVH_OPTIONS)` + `bcvhOptions.map(` replacing a stale `bcvhOptions={ROUTE_BCVH_OPTIONS}` prop check; `aria-pressed={routeType === item.value}` kept, matching the restored attribute). | L3 | Active Test | When touching the route-type/BCVH filter controls. | High |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 51 (appended 2026-08-29) records this remediation, `READY FOR INDEPENDENT TECHNICAL REVIEW`. Section 50 (Phase I1 remediation) and Section 49 (original BLOCKED finding) stand unchanged as historical record. | L2 | Active Onboarding | Current ticket only. | Mandatory |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = AC-09 REMEDIATED / READY FOR INDEPENDENT TECHNICAL REVIEW`. Per `DEC-021`, the same executor implemented and technically validated this round, so it stops here rather than at `READY FOR PO CHECK` — Claude Code does not self-award PO PASS, and the Product Owner's UI Check (Design of Record §12.2) has not been requested. No backend, Evidence, schema, database, or business rule was changed. The disclosed `analysis_date` URL-parameter item remains untouched, out of this round's scope. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 Independent Technical Review - BLOCKED - 2026-08-30

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Section 14 (appended) records the Independent Technical Review at baseline `d138242e`, performed by `Opus` per `DEC-021` and Design §9.5 with a disclosed partial deviation (different model, same session). Confirms `AC-05` — the ticket's highest-risk requirement — correct on **all 9 real BCVH across both periods**, cross-checked against independently written SQL *and* against `getBcvhRanking` itself; 136 contract checks + 90 frontend-merge checks passed with 0 failures, run against the real database through the real production service layer and the real shipped frontend modules; worst end-to-end HTTP 784 ms vs the 1.5 s target; zero database writes. Returns **BLOCKED** on three findings: `ITR-BLOCK-01` `AC-14` not met (literal `MTD` in two ticket-authored files, guard test covers only one frontend file), `ITR-BLOCK-02` default sort coerces `null` to `0` so no-data routes rank as the worst routes (reproduced on real data, 5 of 9 BCVH), `ITR-BLOCK-03` §7.5 panel missing four mandated deliverables including the `daily_series` chart whose data is fetched and discarded. Four non-blocking observations incl. `AC-09` only partially met and an evidence error in Section 12. | L2/L3 | Current Required Reading | Mandatory before any remediation of this ticket and before any PO Check is contemplated. | Mandatory |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 52 (appended 2026-08-30) records the Independent Technical Review, the confirmed `AC-05` result, and the `BLOCKED` outcome with the three findings tabulated. Sections 1-51 stand unchanged as historical record. | L2 | Active Onboarding | Current ticket only. | Mandatory |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-ROUTE-RANKING-PERIOD-01_DESIGN.md` | Design of Record | Unchanged (Revision R1). Re-read for this review; §3.1/`AC-14`, §4.4/`T-01`, §7.5 and §9.5 are the clauses the three blocking findings are measured against. | L2/L3 | Current Required Reading | Mandatory before remediating any of the three findings. | Mandatory |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = INDEPENDENT TECHNICAL REVIEW COMPLETE / BLOCKED — NOT READY FOR PO CHECK`. `INDEPENDENT TECHNICAL PASS` is not awarded; no PO PASS is self-awarded; the Product Owner UI Check (Design §12.2) is not reachable at this state. The verified `AC-05` result stands and does not require re-proving by a remediation round. Remedy scope and executor are a CTO/PO decision and were not self-activated. No product code, backend, Evidence, schema, database, or business rule was changed by this review. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 ITR-BLOCK-02 Remediation - 2026-08-31

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Section 15 (appended) closes `ITR-BLOCK-02` only: `sortRouteRows()` now sorts a route with no data on the anchor day (`day_rate`/`passed_rate: null`) last, regardless of sort direction, instead of coercing it to `0` and ranking it as the worst route; the panel's inline worst-route auto-select reuses the same fixed function. Scoped to exactly the two fields the finding reproduced against — `month_rate`/`previous_month_rate`/`delta` and every count column unaffected, confirmed by a dedicated test. 5 new tests, real-data re-verification of the exact BCVH `535470`/`537015`/`533140` reproduction from §14.3 across all 6 of the 9 BCVH with any absent-on-anchor route, both sort directions. `ITR-BLOCK-01`/`ITR-BLOCK-03` untouched, ticket remains `BLOCKED`. | L2/L3 | Current Required Reading | Mandatory before any further work on this ticket. | Mandatory |
| `frontend/src/features/route/routeRankingCalculations.js` | Data Helper | New `NULLABLE_RATE_SORT_KEYS`/`isMissingRate()`; `sortRouteRows()` sorts a missing-rate row last independent of direction, scoped to `day_rate`/`passed_rate`. | L3 | Active Code | When touching Route Ranking sort logic. | High |
| `frontend/src/features/route/RoutePerformancePage.jsx` | Page | The default-selected-route logic now calls the fixed `sortRouteRows()` instead of a duplicate inline sort with the same `toNumber(null)` defect. | L3 | Active Code | When touching the Route Ranking default-selection logic. | High |
| `frontend/src/features/route/routeRankingCalculations.test.js` | Test | 5 new tests for `ITR-BLOCK-02`: real `0%` first / real rates ascending / no-data always last in both sort directions; the rule applies to the shipped default key `passed_rate`; multiple no-data routes; `month_rate` explicitly confirmed unaffected (scope boundary). | L3 | Active Test | When touching Route Ranking sort logic. | High |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Section 53 (appended 2026-08-31) records the `ITR-BLOCK-02` remediation. Ticket state: `ITR-BLOCK-02 REMEDIATED`, still `BLOCKED` on `ITR-BLOCK-01`/`ITR-BLOCK-03`. Sections 1-52 stand unchanged as historical record. | L2 | Active Onboarding | Current ticket only. | Mandatory |

Current state: `F13-ROUTE-RANKING-PERIOD-01 = ITR-BLOCK-02 REMEDIATED / STILL BLOCKED — NOT READY FOR PO CHECK`. `ITR-BLOCK-01` (`AC-14` literal `MTD`) and `ITR-BLOCK-03` (Design §7.5 panel deliverables) remain open and untouched. No PO PASS self-awarded; the Product Owner UI Check (§12.2) is not requested. No backend, Evidence, schema, database, or business rule was changed. `F13-BCVH-RANKING-OVERVIEW-01` remains `COMPLETED / PO PASS / CLOSED` and is not reopened. `AUTO-BACKFILL-RUNTIME` remains separately open per `PROJECT_SNAPSHOT.md`.

## F13-ROUTE-RANKING-PERIOD-01 ITR-BLOCK-01 Remediation - 2026-08-31

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/06_REVIEWS/Route/F13-ROUTE-RANKING-PERIOD-01-PHASE-I1_CHECKPOINT_001.md` | Checkpoint | Section 16 (appended) closes `ITR-BLOCK-01`: re-scanning the entire ticket-authored file set (not just the two files §14.3 named) found a **third**, previously unreported literal `MTD` in `FactBuuGuiRepository.js`'s `getRoutePeriodPreviousMonth()` comment. All three occurrences reworded to "same-elapsed-days capping formula" prose — no logic, SQL, or behavior change. The prior single-file `AC-14` guard is replaced by two new guard tests (one backend, one frontend) hardcoded against the ticket's real full file footprint, so scope cannot silently narrow again. `rg MTD` over that scope, net of the two guards' own necessary self-reference, returns 0. `ITR-BLOCK-03` untouched, ticket remains `BLOCKED`. | L2/L3 | Current Required Reading | Mandatory before any further work on this ticket. | Mandatory |
| `backend/src/services/routePeriodService.js` | Service | Comment reworded (line 11), no logic change. New guard test (in its test file) scans every backend file this ticket authored. | L3 | Active Code | When touching Route Ranking period service code or its `AC-14` guard. | High |
| `backend/src/repositories/FactBuuGuiRepository.js` | Repository | Comment reworded (line 515, inside this ticket's `getRoutePeriodPreviousMonth()` addition), no logic change. Pre-existing, frozen `getBcvhOverviewMtd()`/`kind === 'mtd'` code (lines 209-210/288) untouched. | L3 | Active Code | When touching Route Ranking period repository code. | High |
| `backend/src/repositories/FactBuuGuiRepository.routePeriod.test.js` | Test | Test name reworded, assertions unchanged. | L3 | Active Test | When touching Route Ranking period repository tests. | High |
| `backend/src/services/routePeriodService.test.js` | Test | New `AC-14` guard test scanning `routePeriodService.js`, `FactBuuGuiRepository.js`, `FactBuuGuiRepository.routePeriod.test.js`, `DashboardController.js`, `DashboardController.routePeriods.test.js`, `f13Routes.js`. | L3 | Active Test | When adding or touching any backend file for this ticket. | High |
| `frontend/src/features/route/routePeriodData.test.js` | Test | New `AC-14` guard test scanning every other frontend file this ticket authored (`F13DashboardClient.js`, `RoutePerformancePage.jsx`, `routeRankingCalculations.js`, and every ticket-authored test file), superseding the prior single-file-only guard. | L3 | Active Test | When adding or touching any frontend file for this ticket. | High |
...
| `docs/10_TICKETS/F13-BCVH-DAILY-SAMEWEEKDAY-01_MANIFEST.md` | Closed Ticket Manifest | PO-authorized amendment adding a 10th column "Tăng/giảm so với cùng kỳ (tuần trước)" to the closed F13-BCVH-MONTHLY-CUMULATIVE-01 9-column table; embedded Design of Record, implementation, test evidence, and Section 7 records the Product Owner's explicit UI PASS at accepted commit `f4a06ea`. | L2 | `IMPLEMENTED / PO UI PASS / CLOSED` | Historical reference; side ticket, separate from Current Ticket. | Reference |
...
| `docs/10_TICKETS/F13-ROUTE-POSTMAN-IDENTITY-01_MANIFEST.md` | Current Ticket Manifest | Sections 6–8 record DB Buu ta audit results, locked stitching pipeline, and the postman catalog review design proposal. | L2 | `DISCOVERY AUDIT COMPLETE` | Current executor onboarding. | Mandatory |
| `docs/06_REVIEWS/Route/F13-ROUTE-POSTMAN-IDENTITY-01_CHECKPOINT_001.md` | Current Checkpoint | Section 4 records DB Buu ta discovery & reconciliation findings; Section 5 sets completion gate. | L2 | `DISCOVERY AUDIT COMPLETE` | Current executor onboarding. | Mandatory |
...
## F13-ROUTE-POSTMAN-IDENTITY-01 Design of Record v2 (PO D1-D4) — 2026-09-18

Product Owner approved the design direction and locked D1-D4. Documentation only; no product code, schema or data changed.
...
## IMPORT-BULK-REIMPORT-ALL-01 Part B Implementation and PO UI PASS / Closure — 2026-09-22

| Path / Pattern | Type | Purpose Summary | Authority | New Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/10_TICKETS/IMPORT-BULK-REIMPORT-ALL-01_MANIFEST.md` | Ticket Manifest (Sections 13-14 appended) | Section 13 (Antigravity): Part B frontend delivered per DoR v2 -- "Chọn tất cả chưa hoàn tất" preserved, existing "Nhập lại" renamed "Nhập mới" for `INCOMPLETE`/`DATA_ERROR`, new "Chọn tất cả" (incl. `COMPLETED`, LỊCH NGHỈ always excluded) backed by a separate `selectedReimportKeys` state, mandatory acknowledgment checkbox before reimporting completed dates. Section 14: Product Owner UI check PASS confirmed in chat; ticket closed `IMPLEMENTED / PO UI PASS / CLOSED`, citing both Part A (`0e16c99`, `a725c1b`, `5387101`) and Part B (`9c9e09b`) accepted commits. | L2 | `IMPLEMENTED / PO UI PASS / CLOSED` | Final record for this ticket; no further work without a new ticket or explicit reopening. | Mandatory |
| `docs/06_REVIEWS/Import/IMPORT-BULK-REIMPORT-ALL-01_CHECKPOINT_001.md` | Ticket Checkpoint (Section 16 appended; header updated) | Records the closure pointer to Manifest Section 14; the checkpoint itself remains the read-only audit trail plus the two independent Part A reviews, unedited historically. | L2 | `CLOSED (pointer only)` | Reference only. | Reference |

Product Owner instruction received in chat (2026-09-22): explicit confirmation of Part B UI PASS ("PO đã kiểm tra giao diện và xác nhận PASS Part B của IMPORT-BULK-REIMPORT-ALL-01"). No Browser/Playwright used by this closure step; no product code changed; no queue action or reimport run against real data. Current Ticket (`F13-ROUTE-POSTMAN-IDENTITY-01`) is unaffected; no other ticket's scope was touched.

## F13-BCVH-WEEKLY-COMPARISON-01 Implementation — 2026-09-22

Product Owner requested a new business requirement in chat (weekly quality-comparison table in BCVH Ranking), confirmed a custom Thursday-Wednesday week convention after a read-only audit flagged a mismatch with ISO 8601 weeks, then authorized full implementation. Claude Code (Sonnet 5) implemented backend, frontend, and tests on branch `codex/da-impl-006`, stopping at `READY FOR PO UI CHECK`.

| Path / Pattern | Type | Purpose Summary | Authority | Status | When Read | Importance |
| --- | --- | --- | --- | --- | --- | --- |
| `docs/10_TICKETS/F13-BCVH-WEEKLY-COMPARISON-01_MANIFEST.md` | Ticket Manifest (new) | PO authorization, embedded implementation record (files changed, test evidence, live-database performance and correctness checks against the PO's exact worked examples), and the PO UI Check checklist. | L2 | `IMPLEMENTED / TECH PASS / READY FOR PO UI CHECK` | Required reading for this ticket and any future delta to the weekly-comparison block. | Mandatory |
| `docs/04_TECHNICAL_PLANNING/Feature/F13-BCVH-WEEKLY-COMPARISON-01_DESIGN.md` | Design of Record (new) | The Thursday-start/ISO-numbered week math (with worked examples matching the PO's Tuần 36/37/38), the two new endpoint contracts, the data-driven "never claim data not yet imported" display rule, and measured real-database performance. | L2 | `IMPLEMENTED / TECH PASS` | Before touching the week-boundary math or either new endpoint. | Mandatory |

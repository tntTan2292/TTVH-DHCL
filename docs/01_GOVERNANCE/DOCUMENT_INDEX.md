...
- Current ticket (`2026-08-28`): `AB-CALENDAR-01` (4-status PO coverage model), `IMPLEMENTED / TECHNICAL PASS / READY FOR PO CHECK`. All four design decisions implemented as approved; PO UI check pending after a backend restart, no PO acceptance is claimed. `AUTO-BACKFILL-RUNTIME` (`ACTIVATED / READY FOR PO CHECK`, 2026-08-20, baseline `a135f799`) remains open and unchanged; no automatic real run is authorized by that readiness state.
- Current manifest: `docs/10_TICKETS/AUTO-BACKFILL-RUNTIME_MANIFEST.md` (AB-CALENDAR-01 is Section 39).
- Current design of record: `docs/04_TECHNICAL_PLANNING/Feature/AB-CALENDAR-01_4_STATUS_MODEL_DESIGN.md` (the holiday calendar design remains valid except its PO decisions 2 and 4, now superseded).
- Current checkpoint: `docs/06_REVIEWS/Import/AUTO-BACKFILL-RUNTIME_CHECKPOINT_001.md`.
- Next required action: Product Owner decides the disposition of the existing four running runs / 472 queued jobs. After clean unblocking, PO tests exactly one date and one source with per-row "Nhập lại", including `WAITING_AUTH`/explicit-Resume behavior, before any expansion. See `PROJECT_SNAPSHOT.md` for live-state detail.

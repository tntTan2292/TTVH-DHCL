...
- Current ticket: `AUTO-IMPORT-014` — general HUE/TCT session-lifecycle reliability hardening, `PO RUNTIME PARTIAL PASS / TCT RE-UPDATE FAIL — FIXED / AWAITING TARGETED PO RECHECK`. Phase 2 (per-source mutex, generalized `activeOperation` exemption, bounded-retry classification, multi-page rebind, ownership-scoped reconciliation for both HUE/TCT) was runtime-tested by the Product Owner on the real machine (commit `0c5d02c3`): HUE login/Import/Update, HUE Re-update session reuse, TCT login/Import/Update, and HUE→TCT→HUE all `PASS`. One `FAIL`: TCT, exactly one date selected, "Re-Update (1)" clicked, returned `DUPLICATE_DATES`. Root cause found and fixed, frontend-only: `frontend/src/main.jsx`'s `<StrictMode>` double-invokes `toggleTctSelectedDate`'s outer `setState` updater; a nested `setTctRefreshDates` side effect inside it unconditionally appended the date on each invocation, compounding one click into a real 2-element duplicate in `refresh_dates`, while the outer, idempotent `tctSelectedDates` computation stayed correct at length 1 (matching the reported "(1)" label) — this is genuinely frontend-created duplication, not a backend normalization defect or stale/merged queue state, and the identical latent bug exists symmetrically in HUE's shared `toggleDateSelection()` helper (fixed there too, same minimal change, not a scope expansion). Backend `normalizeOptionalDates()` in `tctF13BackfillService.js` is unchanged and verified by a new test to still reject a genuine duplicate — validation was not weakened. Regression coverage added for exactly-one-date, multiple-distinct-dates, a direct simulation of the StrictMode double-invocation, re-click-toggles-off, and stale-state-preserved scenarios. Does not reopen `AUTO-IMPORT-013` or touch Phase 2's session-lifecycle code (commit `0d959eb`), both untouched. No `PO PASS`/`CLOSED` — only a targeted recheck (TCT Re-Update for `2026-08-07`) is requested; the rest of the checklist already passed and is not being re-requested. `NETWORK-MANAGEMENT-001` is `PAUSED` at its exact current state by explicit Product Owner instruction — its Phase 4 Technical PASS status (manifest §6 items, 2026-08-08) is untouched, not rolled back (see `docs/01_GOVERNANCE/PROJECT_SNAPSHOT.md` "Paused Ticket State" section for its frozen detail).
- Current manifest: `docs/10_TICKETS/AUTO-IMPORT-014_MANIFEST.md`.
- Current checkpoint: `docs/06_REVIEWS/Import/AUTO-IMPORT-014_CHECKPOINT_003.md` (Phase 2 implementation: `_CHECKPOINT_002.md`; Phase 1 discovery: `_CHECKPOINT_001.md`).
- Next required action: Product Owner rechecks TCT Re-Update for `2026-08-07` only; only after that record `PO PASS`/`CLOSED`. Resume `NETWORK-MANAGEMENT-001` (paused, not closed) only on separate explicit Product Owner authorization; do not activate any other next ticket without one.
...
| `docs/10_TICKETS/NETWORK-MANAGEMENT-001_MANIFEST.md` | Ticket Manifest | Active ticket NETWORK-MANAGEMENT-001: locked four-phase plan (Nền tảng, Ba bản đồ, Import, Nghiệm thu) for Mạng điểm phục vụ, Mạng đường thư cấp 2, Sơ đồ tuyến phát. Phase 1 `COMPLETED / TECHNICAL PASS`, PO Gate 1 PASS (Section 16); Phase 2 `COMPLETED / PO PASS / CLOSED`, PO Gate 2 PASS (Section 17, 26); Phase 3 `COMPLETED / PO PASS / CLOSED`, PO Gate 3 PASS after a first RUNTIME FAIL and remediation chain (Section 27 closure, Section 28 remediation, Section 29 PO Gate 3 PASS); Phase 4 (Nghiệm thu) `IN PROGRESS` — first scoped item, Sơ đồ tuyến phát data contract audit + remediation, technically complete (Section 30), awaiting PO runtime recheck. | L2 | Active Onboarding (Current Manifest) | Every fresh session while this ticket is active. | Mandatory |
| `docs/06_REVIEWS/Shared/NETWORK-MANAGEMENT-001_CHECKPOINT_001.md` | Checkpoint | Current checkpoint for NETWORK-MANAGEMENT-001: program/Phase state, PO-confirmed baseline, allowed/locked scope, reusable architecture notes, Phase 1 implementation evidence (Section 12), Phase 2 implementation evidence (Section 13), Phase 3 Import/Export/History/Rollback implementation evidence (Section 14), PO Gate 3 Runtime Remediation (Section 15), ĐTC2 journey visual + arrow remediation (Sections 16-17), PO Gate 3 PASS closure (Section 18), Phase 4 Sơ đồ tuyến phát data contract audit + remediation (Section 19). | L2 | Active Onboarding (Current Checkpoint) | Every fresh session while this ticket is active. | Mandatory |
| `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` | Ticket Manifest | Program F13-STANDARDIZATION-001: locked five-phase F1.3 module standardization plan. Tuyến Ranking (Route Ranking) delta `COMPLETED / PO PASS / CLOSED` (Section 16, 2026-08-04); Phase 0 implemented, not separately closed; Phases 1-4 `PLANNED / NOT ACTIVE`. | L2 | Conditional Reference | When F13-STANDARDIZATION-001 scope, the Route Ranking delta closure, or Phase 1-4 planning is needed. | High |
| `docs/06_REVIEWS/Shared/F13-STANDARDIZATION-001_CHECKPOINT_001.md` | Checkpoint | Program checkpoint: program/Phase state, baseline, allowed/locked scope, required reading, and the Route Ranking delta closure record (Section 11). | L2 | Conditional Reference | When F13-STANDARDIZATION-001 scope or the Route Ranking delta closure evidence is needed. | High |

...
| `docs/04_TECHNICAL_PLANNING/Feature/AB-CALENDAR-01_4_STATUS_MODEL_DESIGN.md` | Feature Design | Current design of record for AB-CALENDAR-01 (2026-08-27): the 4 PO-facing coverage statuses (COMPLETED / INCOMPLETE / EXCLUDED / DATA_ERROR) replacing the frozen 6-state model, plus PO's operational-completion delta (processed = COMPLETED+EXCLUDED, unprocessed = INCOMPLETE+DATA_ERROR). Carries the full mapping, queue and selection rules, measured live impact, file scope, test plan, acceptance criteria and the open decisions D1-D3. Supersedes PO decisions 2 and 4 of the holiday calendar design. Implementation not started. | L2 | Current Required Reading | Current ticket only. | Mandatory |

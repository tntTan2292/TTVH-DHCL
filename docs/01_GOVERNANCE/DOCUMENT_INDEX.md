...
- Current ticket: `NETWORK-MANAGEMENT-002` — Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2, `READY FOR PO RECHECK` (2026-08-11). Product Owner reported RUNTIME FAIL on the first delivered version (marker/route overlap too dense; Điểm phục vụ layer only had a whole-layer toggle). Remediated same day, `IntegratedMap.jsx` only: independent per-Loại-điểm checkboxes derived from real data (never hand-typed), default all-on, state persists across the layer's on/off toggle, marker filter and legend badges share one filter helper. Technically validated (oxlint, vite build, 53/53 relevant tests, real-browser: all-on/1-category/multi-category/all-off/category+status+Tạm dừng combos all count-exact, ĐTC2 select/deselect re-verified). `ServicePointsMap.jsx`/`Level2RoutesMap.jsx`/backend/schema confirmed untouched. No PO PASS declared. `NETWORK-MANAGEMENT-001` (closed same day this ticket activated) is not reopened.
...
- Next required action: await Product Owner runtime RECHECK per the checklist provided in the corresponding chat report; do not declare PO PASS or close the ticket without it. `NETWORK-MANAGEMENT-001` (last closed: full program, `COMPLETED / PO FINAL PASS / CLOSED`, 2026-08-10) must not be reopened without new explicit Product Owner authorization.
...
| `docs/10_TICKETS/NETWORK-MANAGEMENT-002_MANIFEST.md` | Ticket Manifest | Active ticket NETWORK-MANAGEMENT-002 (Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2): locked scope (Section 5), discovery (Section 7), implementation options (Section 8), UI/navigation/access-control plan (Section 9), test plan (Section 11), implementation record (Section 14), PO runtime fail + remediation record (Section 15). `READY FOR PO RECHECK` — no PO PASS declared. | L2 | Active Onboarding (Current Manifest) | Every fresh session while this ticket is active. | Mandatory |
| `docs/06_REVIEWS/Shared/NETWORK-MANAGEMENT-002_CHECKPOINT_001.md` | Checkpoint | Current checkpoint for NETWORK-MANAGEMENT-002: ticket state, baseline (`5c0ff1bc`), allowed/locked scope, discovery findings (Section 11), implementation + validation record (Section 12), PO runtime fail + remediation record (Section 13). `READY FOR PO RECHECK`. | L2 | Active Onboarding (Current Checkpoint) | Every fresh session while this ticket is active. | Mandatory |

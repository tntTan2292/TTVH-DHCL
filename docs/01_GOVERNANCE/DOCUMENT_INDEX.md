...
- Current ticket: `F13-STANDARDIZATION-001` (existing program group) — Evidence Consolidation Phase 1, `IMPLEMENTED / READY FOR PO CHECK` (2026-08-11, implementation commit `b147df7c`, plan approved at `34f42c57`). Backend-only, additive-only: `f13DashboardService.getEvidenceList()`'s mapper now passes through `ma_tuyen`/`ten_tuyen`/`ma_bcvh`/`ten_bcvh`, fixing the "Tất cả tuyến" defect where every row displayed "Tất cả tuyến" as its route. No frontend file touched (existing consumers already preferred the real API value). Product Owner approved the plan and Phase 1 only, plus 5 decisions locked for Phase 2/3 (screen name, violation-group carry-over from Tuyến Ranking, no Action Center button yet, frozen-doc amendment requires its own separate governance delta, reconciliation via unique `ma_bg` sets — implemented via new tests). Targeted evidence suite 20/20; full backend sweep 111/115 (4 pre-existing failures, unchanged); full frontend sweep 256/269 (unchanged, no frontend file touched), reported as the true full-suite figure. Most recently closed delta: the Evidence/Chi tiết bưu gửi delta — `CLOSED / PO RUNTIME CHECK PASS` (2026-08-11, commit `a66fa57d`), unaffected. Most recently closed full ticket: `NETWORK-MANAGEMENT-002`, `COMPLETED / PO PASS / CLOSED` (2026-08-11), unaffected.
- Current manifest: `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` (Section 22, Phase 1).
- Current checkpoint: `docs/06_REVIEWS/Shared/F13-STANDARDIZATION-001_CHECKPOINT_001.md` (Section 17, Phase 1) — full record: `docs/06_REVIEWS/Shared/F13-EVIDENCE-CONSOLIDATION-PLAN_CHECKPOINT_001.md` Section 12; preceding audit: `docs/06_REVIEWS/Shared/F13-EVIDENCE-PRODUCT-VALUE-AUDIT_CHECKPOINT_001.md`.
- Next required action: Product Owner runtime check on Phase 1. Phase 2 additionally requires its own separate governance delta for the frozen-document amendment before it may start. `F13-SHIPMENT-001` (`stash@{0}`) remains `DEFERRED / PRESERVED`; `F13-SURFACE-CLEANUP-PLAN` remains not created; `F13-STANDARDIZATION-001` Phases 1-4 of the original 5-phase program plan remain `PLANNED / NOT ACTIVE`. `NETWORK-MANAGEMENT-001` and `NETWORK-MANAGEMENT-002` remain fully closed and must not be reopened without new explicit Product Owner authorization.

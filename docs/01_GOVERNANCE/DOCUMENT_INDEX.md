...
- Current ticket: `NETWORK-MANAGEMENT-002` — Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2, `DISCOVERY + PLANNING COMPLETE` (2026-08-10), no product code changed yet. Activated by explicit Product Owner instruction the same day `NETWORK-MANAGEMENT-001` closed `COMPLETED / PO FINAL PASS / CLOSED` — that program is not reopened. Locked scope: one integrated read-only map screen; data read directly from the two existing source modules (Mạng điểm phục vụ, Mạng đường thư cấp 2), no duplication; independently toggleable layers; the two source screens' already-approved marker/popup/legend/routing/interaction preserved verbatim; no new database table; no dedicated Import/Export/History/Rollback; no behavior change to the two original screens; no Shipment Detail/Evidence merge. Read-only discovery found both source screens already read via existing admin+viewer-readable API endpoints (zero backend/schema work needed) and proposed 2 implementation options, recommending zero-touch-to-originals by default.
- Current manifest: `docs/10_TICKETS/NETWORK-MANAGEMENT-002_MANIFEST.md`.
- Current checkpoint: `docs/06_REVIEWS/Shared/NETWORK-MANAGEMENT-002_CHECKPOINT_001.md`.
- Next required action: await explicit Product Owner/CTO go-ahead to begin implementation (default approach: manifest Section 8's Option B). `NETWORK-MANAGEMENT-001` (last closed: full program, `COMPLETED / PO FINAL PASS / CLOSED`, 2026-08-10) must not be reopened without new explicit Product Owner authorization.
...
| `docs/10_TICKETS/NETWORK-MANAGEMENT-002_MANIFEST.md` | Ticket Manifest | Active ticket NETWORK-MANAGEMENT-002 (Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2): locked scope (Section 5), locked out-of-scope (Section 6), discovery findings (Section 7), 2 implementation options with a recommended default (Section 8), UI/navigation/access-control plan (Section 9), expected file changes (Section 10), test plan (Section 11). `DISCOVERY + PLANNING COMPLETE`, no product code changed. | L2 | Active Onboarding (Current Manifest) | Every fresh session while this ticket is active. | Mandatory |
| `docs/06_REVIEWS/Shared/NETWORK-MANAGEMENT-002_CHECKPOINT_001.md` | Checkpoint | Current checkpoint for NETWORK-MANAGEMENT-002: ticket state, baseline (`5c0ff1bc`), allowed/locked scope, exact next action (awaiting PO/CTO go-ahead), discovery findings (Section 11). | L2 | Active Onboarding (Current Checkpoint) | Every fresh session while this ticket is active. | Mandatory |

...
- Current ticket (`2026-08-27`): `AB-CALENDAR-01` (LỊCH NGHỈ), `BACKEND IMPLEMENTED`. Product Owner approved all four open design decisions before implementation; no UI shipped and no PO acceptance is claimed. `AUTO-BACKFILL-RUNTIME` (`ACTIVATED / READY FOR PO CHECK`, 2026-08-20, baseline `a135f799`) remains open and unchanged; no automatic real run is authorized by that readiness state.
...
| `docs/04_TECHNICAL_PLANNING/Feature/AB-CALENDAR-01_4_STATUS_MODEL_DESIGN.md` | Feature Design | Current design of record for AB-CALENDAR-01 (2026-08-27): the 4 PO-facing coverage statuses (COMPLETED / INCOMPLETE / EXCLUDED / DATA_ERROR) replacing the frozen 6-state model, plus PO's operational-completion delta (processed = COMPLETED+EXCLUDED, unprocessed = INCOMPLETE+DATA_ERROR). Carries the full mapping, queue and selection rules, measured live impact, file scope, test plan, acceptance criteria, and all four approved decisions D1-D4 (narrow include_excluded opt-in; LEGACY_DATA_PRESENT_WITHOUT_EVIDENCE maps to COMPLETED; frozen UI plan amended 6 to 4; DATA_ERROR stays selectable). Supersedes PO decisions 2 and 4 of the holiday calendar design. No design question remains open; implementation not started. | L2 | Current Required Reading | Current ticket only. | Mandatory |

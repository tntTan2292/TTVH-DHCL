...
| `docs/06_REVIEWS/Route/F13_ROUTE_RANKING_REDESIGN_PLAN_CHECKPOINT_001.md` | Route Checkpoint | Binding locked design scope for Route Ranking redesign: data-discovery evidence, PO-approved/CTO-finalized decisions, MVP, Deferred scope, and forbidden inferences. | L2/L3 | Active Onboarding | Current implementation ticket only — read before writing any code. | Mandatory |
| `docs/10_TICKETS/F13-ROUTE-RANKING-REDESIGN-IMPL_MANIFEST.md` | Ticket Manifest | Current F1.3 Route Ranking redesign implementation ticket; authorized scope, forbidden-inference list, executor Claude Code-Sonnet, PO UI Check required before closure. | L2 | Active Onboarding | Current implementation ticket only. | Mandatory |

...
- Current ticket: `F13-STANDARDIZATION-001` (existing program group) — Evidence/Chi tiết bưu gửi delta, `READY FOR PO CHECK` (2026-08-11, commit `a66fa57`). `ShipmentPerformancePage.jsx` is now the canonical `/f13/evidence` component (`admin`+`viewer`); `/f13/ranking/shipment` redirects preserving the query string; real BCVH/date/route defaults; "Tất cả tuyến" support; `fetchAllEvidenceRows()` removes the previous silent 1,000-row cap. Most recently closed full ticket: `NETWORK-MANAGEMENT-002` — Bản đồ tích hợp Điểm phục vụ + Đường thư cấp 2, `COMPLETED / PO PASS / CLOSED` (2026-08-11), unaffected.
- Current manifest: `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` (Section 18, implementation).
- Current checkpoint: `docs/06_REVIEWS/Shared/F13-STANDARDIZATION-001_CHECKPOINT_001.md` (Section 13, implementation).
- Next required action: Product Owner runtime check per the 11-item checklist in manifest Section 18. `NETWORK-MANAGEMENT-001` and `NETWORK-MANAGEMENT-002` remain fully closed and must not be reopened without new explicit Product Owner authorization.

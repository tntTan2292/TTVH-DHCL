...
- Current ticket: `F13-STANDARDIZATION-001` (existing program group) — Evidence Consolidation plan, `PLAN COMPLETE / AWAITING PO APPROVAL` (2026-08-11, baseline `e2c32178`). Planning only, no product code changed, no frozen document edited. Product Owner decided the preceding audit: keep `/f13/evidence` as the single shared violation-detail screen, Tuyến Ranking leads into it, streamline per the audit, remove Recommendation, controlled frozen-doc amendment may be planned. Three new defects found while planning: the evidence-list API discards route/BCVH identity so "Tất cả tuyến" cannot attribute a violation to a route (blocking); a latent `ReferenceError` in `RoutePerformancePage.jsx:262`; and a corrected frontend test baseline of 256/269 with 13 proven-pre-existing failures. Most recently closed delta: the Evidence/Chi tiết bưu gửi delta — `CLOSED / PO RUNTIME CHECK PASS` (2026-08-11, commit `a66fa57d`), unaffected. Most recently closed full ticket: `NETWORK-MANAGEMENT-002`, `COMPLETED / PO PASS / CLOSED` (2026-08-11), unaffected.
- Current manifest: `docs/10_TICKETS/F13-STANDARDIZATION-001_MANIFEST.md` (Section 21, plan).
- Current checkpoint: `docs/06_REVIEWS/Shared/F13-STANDARDIZATION-001_CHECKPOINT_001.md` (Section 16, plan) — full plan: `docs/06_REVIEWS/Shared/F13-EVIDENCE-CONSOLIDATION-PLAN_CHECKPOINT_001.md`; preceding audit: `docs/06_REVIEWS/Shared/F13-EVIDENCE-PRODUCT-VALUE-AUDIT_CHECKPOINT_001.md`.
- Next required action: Product Owner approval of the Evidence Consolidation plan before any implementation phase starts; the frozen-document amendment list (plan Section 8) requires its own separate explicit approval before plan Phase 2. `F13-SHIPMENT-001` (`stash@{0}`) remains `DEFERRED / PRESERVED`; `F13-SURFACE-CLEANUP-PLAN` remains not created; `F13-STANDARDIZATION-001` Phases 1-4 remain `PLANNED / NOT ACTIVE`. `NETWORK-MANAGEMENT-001` and `NETWORK-MANAGEMENT-002` remain fully closed and must not be reopened without new explicit Product Owner authorization.

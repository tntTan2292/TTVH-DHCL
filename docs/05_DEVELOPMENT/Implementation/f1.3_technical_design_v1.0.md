...
ssot_reference: docs/07_REFERENCE/Domains/domain_quality_management/f1.3_chat_luong_phat_lien_tinh/
version: 1.0.0
---
...
- [x] Schema SQLite được khởi tạo đúng và có Unique Constraint `(ngay_do_kiem, ma_bg)`.
- [x] Hàm import dùng cơ chế bỏ qua trùng lặp ở mức Database thay vì Rollback cả cụm.
- [x] Bảng Log lưu trữ chính xác số lượng file bị reject từng dòng.
- [x] API kpi trả về đúng các key (`tong_buu_gui`, `buu_gui_khong_dat`...) thay vì chỉ trả về `%`.
- [x] Client xử lý format đúng định dạng yêu cầu của QA.

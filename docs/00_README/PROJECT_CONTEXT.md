...
## 3B. Technical Debt (Nợ Kỹ Thuật)
- **TD-001: Rule Configuration**
  - **Tình trạng:** Rule Engine hiện đang hardcode Business Threshold, Priority, Recommendation Message, và Action Message.
  - **Lý do:** Đây là giải pháp tạm thời để hoàn thành nhanh Phase 1.5B cho module F1.3.
  - **Hành động:** Sau khi hoàn thành toàn bộ F1.3, hệ thống sẽ được refactor thành Rule Configuration Center (Cho phép quản lý cấu hình động từ giao diện hoặc DB).

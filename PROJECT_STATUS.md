...
- AUTO-IMPORT-014: `ACTIVE / TECHNICAL GATE PASS / AWAITING PO RUNTIME ACCEPTANCE` (2026-08-08); general HUE/TCT session-lifecycle reliability defect reported after AUTO-IMPORT-013's closure. PO authorized bounded implementation of the Phase 1 remediation design: per-source async mutex (HUE/TCT independent), the old HUE-only queue-running exemption generalized to any owning operation for either source, a bounded retry + login-form-confirmed classification replacing the old single-reading destructive session close, multi-page rebind in the portal client (closing a stray/duplicate window no longer fails the session if a valid authenticated page remains), and orphan-profile reconciliation generalized from TCT-only to both sources. 14 new test blocks added; full existing regression suite passes unchanged; no frontend change needed. No PO PASS declared — a PO Runtime Acceptance Checklist is provided. See `docs/10_TICKETS/AUTO-IMPORT-014_MANIFEST.md`, `docs/06_REVIEWS/Import/AUTO-IMPORT-014_CHECKPOINT_001.md` and `_CHECKPOINT_002.md`.

## Next Ticket

- Current active ticket: `AUTO-IMPORT-014` (emergency, Technical Gate PASS, awaiting PO runtime acceptance).
- Next planned action: `Product Owner performs the runtime acceptance checklist in docs/10_TICKETS/AUTO-IMPORT-014_MANIFEST.md whenever ready; only then should PO PASS/CLOSED be recorded. Resume NETWORK-MANAGEMENT-001 (paused, not closed) only on separate explicit Product Owner authorization.`

...
test('queue controller forwards identity filters and authenticated Admin actor', async () => {
    ...
    assert.deepEqual(input, { indicator: 'F9.TEST', lane: 'HUE', fromDate: null, toDate: null, includeExcluded: false, confirmReplaceCompleted: false, actor: 'tester', roles: ['admin'] });
});
...
test('queue controller forwards from_date/to_date when supplied, and null when omitted', async () => {
    ...
    assert.equal(res.statusCode, 201);
    assert.deepEqual(input, { indicator: null, lane: null, fromDate: '2026-01-01', toDate: '2026-01-31', includeExcluded: false, confirmReplaceCompleted: false, actor: 'tester', roles: ['admin'] });
});
...
test('queue controller forwards confirm_replace_completed as a boolean', async () => {
    let input;
    const controller = new AutoBackfillQueueController({ queueService: {
        async createRun(value) {
            input = value;
    ...
    const res = response();
    await controller.createRun(request({ body: {
        indicator: 'F9.TEST', lane: 'HUE', from_date: '2026-01-03', to_date: '2026-01-03', confirm_replace_completed: true,
    } }), res);
    assert.equal(res.statusCode, 201);
    assert.equal(input.confirmReplaceCompleted, true);
    assert.equal(input.includeExcluded, false);
});

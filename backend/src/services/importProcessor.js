...
// ─────────────────────────────────────────────────────────────────────────────
// File-based Wrapper (backward compatibility for importWatcher.js)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * File-path-based import entry point — used by importWatcher.js.
 *
 * Flow:
 *   1. Validate filename → extractDateFromFilename (SSOT: ngay_do_kiem from filename)
 *   2. Read buffer from disk → parseF13Excel (41-column static mapping)
 *   3. Import to DB → importParsedData (forceReimport: true for file watcher)
 *   4. Move file to Processed or Error directory
 *
 * Signature preserved from original importProcessor.js to avoid breaking importWatcher.js.
 *
 * @param {string} filePath
 * @param {string} INCOMING_DIR   (unused — file already at filePath)
 * @param {string} PROCESSED_DIR  destination on success
 * @param {string} ERROR_DIR      destination on failure
 */
async function processImportFile(filePath, INCOMING_DIR, PROCESSED_DIR, ERROR_DIR) {
    ...
    const moveFile = (destDir) => {
        try {
            ...
            console.error(`[importProcessor] Cannot move file '${filename}': ${e.message}`);
        }
    };

    try {
        // Step 1: Validate filename format, extract ngay_do_kiem (SSOT rule)
        ngay_do_kiem = extractDateFromFilename(filename);

        // Step 2: Read and parse Excel
        const buffer     = fs.readFileSync(filePath);
        const { parsedData } = parseF13Excel(buffer);

        // Step 3: Import — file watcher always force-reimports (auto-confirms overwrite)
        ...
        console.log(
            `[importProcessor] SUCCESS | ${filename} | ` +
            `total=${result.total}, inserted=${result.inserted}, duplicates=${result.errors}`
        );

        moveFile(PROCESSED_DIR);

    } catch (error) {
        console.error(`[importProcessor] FAILED  | ${filename} | ${error.message}`);
        moveFile(ERROR_DIR);
    }
}

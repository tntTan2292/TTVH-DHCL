...
const authCodeOld = `            const profileDir = process.env[sourceConfig.profileDirEnv] || sourceConfig.defaultProfileDir();
...
const authCodeNew = `            const profileDir = process.env[sourceConfig.profileDirEnv] || sourceConfig.defaultProfileDir();
...
const recoverCodeOld = `    async recover(source) {
...
const recoverCodeNew = `    async recover(source) {
        const sourceConfig = this.normalizeSource(source);
        const profileDir = process.env[sourceConfig.profileDirEnv] || sourceConfig.defaultProfileDir();
        const entry = getOrCreateRegistryEntry(sourceConfig.source);
...
code = code.replace(recoverCodeOld, recoverCodeNew);
